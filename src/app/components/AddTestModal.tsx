...
export function AddTestModal({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isNaN(numValue) || numValue <= 0) {
      setError('Bitte geben Sie einen korrekten Wert ein');
      return;
    }
    ...
    if (selectedDate > new Date()) {
      setError('Datum kann nicht in der Zukunft liegen');
      return;
    }
        ...
        <>
          ...
          >
            ...
            <div className="sticky top-0 bg-zinc-900 px-6 py-4 border-b border-zinc-800 flex items-center justify-between">
              <h2 className="text-xl font-semibold">Ergebnis hinzufügen</h2>
              <button
                onClick={onClose}
            ...
            <form onSubmit={handleSubmit} className="p-6">
              {/* Выбор параметра */}
              <div className="mb-6">
                <label className="block text-sm text-zinc-400 mb-3">Analyseparameter</label>
                ...
                {onOpenAddParameter && (
                  <button
                    ...
                    className="w-full mt-2 px-4 py-3 rounded-xl flex items-center gap-3 transition-all bg-zinc-800/50 border-2 border-transparent hover:bg-zinc-800"
                  >
                    <PlusCircle className="w-5 h-5 text-zinc-500" />
                    <p className="font-medium">Neuen Parameter hinzufügen</p>
                  </button>
                )}
              </div>
              ...
              <div className="mb-6">
                ...
                {value && isWithinNormalRange() !== null && (
                  <div className="mt-2 flex items-center gap-2">
                    ...
                    >
                      {isWithinNormalRange() ? '✓ Im Normalbereich' : '⚠ Außerhalb des Bereichs'}
                    </div>
                  </div>
                )}
              </div>
              ...
              {/* Дата */}
              <div className="mb-6">
                <label className="block text-sm text-zinc-400 mb-2">Analysedatum</label>
                <div className="relative">
